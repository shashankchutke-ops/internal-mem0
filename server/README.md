...
This repository includes a standalone server image. Run Compose from this
directory; it does not require a checkout of the upstream Mem0 monorepo. The
API and dashboard are published on the host for local access, while
PostgreSQL/pgvector stays on the private Compose network.
...
For remote access, point Cloudflare Tunnel ingress at the API on
`http://localhost:8888` and the dashboard on `http://localhost:3000`. Keep the
PostgreSQL service private; it has no host port mapping in this deployment.
...
- **Configuration** — runtime LLM and embedder override. Changes persist to the app database and reapply on restart, layered over the values from your `.env`. With the pgvector store, the server aligns the bundled embedder's output dimensions with the store's 1536-dimension column so changing to Gemini does not make writes fail after reads succeed.
